...
def update_logged_table(conn):
    with conn.cursor() as cursor:
        cursor.execute(f"""
                        ALTER TABLE {TableName} SET LOGGED;
                """)
        print(f"Set {TableName} to Logged")
...
def add_index(conn):
    with conn.cursor() as cursor:
        cursor.execute(f"""
                ...
                """)
        print(f"Adding indexes {TableName}")


def update_temp_table(conn):
    with conn.cursor() as cursor:
        cursor.execute(f"""
...
def main():
    ...
    if CreateDB:
        createTable(conn)
    load(conn, cmdlist)
    update_temp_table(conn)
    if CreateDB:
        update_logged_table(conn)
        add_index(conn)
